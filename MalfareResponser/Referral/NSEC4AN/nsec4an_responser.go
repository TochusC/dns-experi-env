...
type NSEC4ANResponser struct {
	ServerConf    godns.DNSServerConfig
	DNSSECManager godns.DNSSECManager
}

func (r *NSEC4ANResponser) Response(connInfo godns.ConnectionInfo) (dns.DNSMessage, error) {
	// 解析查询信息
	...
	if qLabelsNum == 2 {
		...
		if len(qLabels) == 3 {
			...
			if qLabels[1][:4] == "nxns" {
				...
				// xxx.xxxx(not nxns*).test NXDOMAIN
