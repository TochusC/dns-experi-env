...
type CXDResponser struct {
	ServerConf    godns.DNSServerConfig
	DNSSECManager godns.DNSSECManager
}

func (r *CXDResponser) Response(connInfo godns.ConnectionInfo) (dns.DNSMessage, error) {
	// 解析查询信息
	...
	switch qType {
	case dns.DNSRRTypeA:
		if qLables[0] == "ns" || qLables[0] == "www" || qLables[0][:4] == "cname" {
			// 进行CNAME重定向
			...
			rr := dns.DNSResourceRecord{
				...
				TTL:   86400,
				RDLen: 0,
				RData: &dns.DNSRDATANS{NSDNAME: fmt.Sprintf("ns.%s", qName)},
			}
			resp.Answer = append(resp.Answer, rr)
		}
