...
type KeyTrapManager struct {
	...
	KSKTagMap map[string]map[int]dns.DNSRDATADNSKEY
}
...
func (m KeyTrapManager) CreateDNSSECMaterial(zName string) godns.DNSSECMaterial {
	...
	mKeyPriv := []byte{}
	if _, ok := m.KSKTagMap[zName]; !ok {
		m.KSKTagMap[zName] = make(map[int]dns.DNSRDATADNSKEY)
	}
	...
	for i := 0; i < m.AttackVec.DS_KSK_PairNum; i++ {
		...
		if _, ok := m.KSKTagMap[zName][int(kSKTag)]; ok {
			i--
			continue
		} else {
			m.KSKTagMap[zName][int(kSKTag)] = *pubKSK.RData.(*dns.DNSRDATADNSKEY)
...
func (m KeyTrapManager) EstablishToC(qry dns.DNSMessage, resp *dns.DNSMessage) error {
	...
	if qType == dns.DNSRRTypeDNSKEY {
		...
		for _, kRDATA := range m.KSKTagMap[qName] {
...
func (r *KeyTrapResponser) Response(connInfo godns.ConnectionInfo) (dns.DNSMessage, error) {
	...
	fmt.Printf("Response: response size: %d\n", resp.Size())
	return resp, nil
}
	...
	server := godns.GoDNSServer{
		...
		Responer: &KeyTrapResponser{
			...
			DNSSECManager: KeyTrapManager{
				...
				DNSSECMap:  tAnchor,
				AttackVec:  ExperiVec,
				KSKTagMap:  make(map[string]map[int]dns.DNSRDATADNSKEY),
			},
		},
	}
	server.Start()
